...
- **Trailer de Coautoria:** Todos os commits gerados por IA recebem `Co-Authored-By: Gitpr-cli <gitpr@natanfiuza.dev.br>` — idempotente (nunca duplica, preserva trailers de terceiros), oculto da TUI e com opt-out `GITPR_COAUTHOR=false`.
- **Fix do Hang do MCP Server:** Os 12 tool handlers eram síncronos e corriam inline no event loop — qualquer chamada bloqueante congelava o servidor stdio. Novo decorator `_offload` (anyio worker threads), warm-import no startup, `stdin=subprocess.DEVNULL` em todos os subprocessos e timeout duro de 10s no download de smart-excludes. Testes e2e novos com JSON-RPC stdio real.
- **Correções do Modal de Erro do Linter:** Botões "Commit with --no-verify" e "Abort" lado a lado (antes empilhados e sobrepostos); a escolha no-verify passa a retomar o fluxo de commit; push do modal adiado via `call_next` para o message pump da app.
...
- **Documentação Multilíngue Expandida:** `docs/ARCHITECTURE.md` reescrito em EN canónico + 4 locales criados (18 tópicos de arquitetura); novo tópico `i18n_explanation` em 5 idiomas; READMEs e 4 tópicos atualizados.
...
Ou transfira o binário standalone em [GitHub Releases](https://github.com/natanfiuza/gitpr/releases).
...
Os seus commits passam a ter o trailer de coautoria automaticamente — desative com `GITPR_COAUTHOR=false`.

## Dicas úteis

Execute novamente qualquer comando de IA sem alterar o código e o GitPR responde em milissegundos: as respostas ficam em cache em `~/.gitpr/cache/prompts/`, indexadas por um hash MD5 do diff + instruções — repetir um comando não gasta nada da sua quota de API.
