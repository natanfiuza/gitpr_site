...
Detecção de primeira execução, configuração interativa de chaves API, configuração `.env` em `~/.gitpr/`. Roteamento de comandos para todas as flags (`--commit`, `--review`, `--fullreview`, `--linter`, `--skill`, `--issue`, `--blame`, `--chat`).
...
Geração de chave Fernet (`secret.key`), funções `encrypt_data` e `decrypt_data`. Chaves de API nunca armazenadas em texto puro.
...
TUI interativa com memória de mensagens por branch. F5 extrai blocos de código para arquivos de patch. F6 exporta sessões para Markdown. Slash commands para ações comuns.

### 7. Internacionalização (`src/i18n.py`)
Helper `__()` inspirado no Laravel com placeholders nomeados. Pacotes de tradução JSON em `~/.gitpr/langs/`. Fallback para inglês para chaves ausentes. Suporta `en`, `pt_br`, `pt_pt`, `fr`, `es`.
...
- **Provedores IA:** 3 (Gemini, DeepSeek, Ollama)
...
- **Segurança:** Criptografia simétrica Fernet (AES-128-CBC)
