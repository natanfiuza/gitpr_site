...
### 2. Inicializar num Repositório

Para configurar o GitPR numa pasta de um novo repositório, execute:

```bash
gitpr --install
```

A configuração guiada transfere templates de skills, instala Git Hooks, configura o MCP para os seus editores e verifica a chave de API do seu fornecedor de IA.
...
- **`--hook <ficheiro>`** — Usado internamente pelos Git Hooks para injetar mensagens de commit diretamente no ficheiro temporário do Git.
- **`--pre-save`** — Flag de debug que guarda o payload completo da IA (instrução do sistema + prompt) num ficheiro JSON antes de cada chamada. Combine com qualquer comando de IA (ex: `gitpr -c --pre-save`).
:::
...
O GitPR deteta automaticamente o idioma do seu sistema. Suporta **5 idiomas** (PT-PT, EN, PT-BR, ES, FR), com traduções transferidas automaticamente. Force um idioma com `--lang pt_pt` ou defina `GITPR_LANG` na sua configuração.
