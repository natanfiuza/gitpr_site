# **🚀 Relatório de Estado do Projeto: GitPR CLI — v0.0.32 (2026-08-06)**
...
- **Expansão da Cobertura i18n (491 chaves):** Sincronização completa das chamadas `__()` em `core.py`, `main.py` e `linter_engine.py` com o ficheiro de tradução `pt_br.json`. Script de verificação `tests/sync_i18n.py` para detetar chaves órfãs em qualquer ficheiro fonte. Adição de 5 novas traduções para strings do Smart Excludes, banner CLI com `--install` e telemetria local.
- **Smart Excludes para Documentação:** Filtro pathspec inteligente que deteta e exclui ficheiros de documentação (`.md`, `.rst`, `.txt`) do diff, com notificação visual da quantidade de ficheiros excluídos (`📄 {count} documentation file(s) excluded`) e link para a documentação.
- **Sincronização Automática de Hooks Git:** Sistema de versionamento independente para scripts de hook (`__scripts_version__` v0.0.1) com verificação e atualização automáticas ao executar `--installhooks`. Deteta o idioma do ambiente e descarrega a versão correta dos templates.
...
- **Cache i18n com Indexação por Idioma:** O sistema de cache de respostas IA inclui agora o idioma corrente no chaveamento, o que evita colisões entre respostas geradas em idiomas diferentes.
- **Versionamento Centralizado no Updater:** A versão do GitPR (`__version__`) e a versão dos dicionários de idioma (`__lang_version__`) são agora derivadas exclusivamente do `src/updater.py`, o que elimina a duplicação com `pyproject.toml`.
- **Documentação de Padrões de Arquitetura:** Memory index com 14 padrões documentados extraídos de 36 relatórios de tarefas, que cobre cache, spinner, MCP, métricas, UI, versionamento e outros subsistemas.
...
* **Smart Excludes 🆕:** Filtro de pathspec inteligente (`gitpr.smart-excludes.json`) remoto — descarregado do GitHub e atualizado automaticamente com versionamento (`SMART_EXCLUDES_VERSION`). **Nova funcionalidade:** exclusão de ficheiros de documentação com notificação visual (`📄 {count} documentation file(s) excluded`) e link `Learn more` para a documentação.
...
* **Métricas de Linter 🆕:** Eventos de execução do linter registados via `log_linter_metric()` com contagem de erros e warnings.
...
* **Versionamento Centralizado 🆕:** `__version__` (0.0.32), `__lang_version__` (v0.0.10), `__scripts_version__` (v0.0.1), `SMART_EXCLUDES_VERSION`, `THINKING_WORDS_VERSION` — todos derivados exclusivamente do `updater.py`, fonte única de verdade para o versionamento.
...
* **Deteção Automática:** Deteta o idioma do SO na primeira execução e salva em `GITPR_LANG`.
* **5 Idiomas:** en_us (padrão/fallback), pt_br, pt_pt, es_es, fr_fr.
* **Ficheiros Versionados:** `__lang_version__` (v0.0.10) controla atualização dos pacotes de idioma (`langs/*.json`).
* **Cobertura Expandida 🆕:** 491 chaves de tradução em pt_BR (+44 desde v0.0.6). Sincronização completa entre chamadas `__()` no código-fonte e dicionários de tradução. Script `tests/sync_i18n.py` para deteção automática de chaves órfãs.
* **Cache com Indexação por Idioma 🆕:** Respostas de IA cacheadas incluem agora o idioma corrente no chaveamento MD5, o que evita colisões entre idiomas diferentes.
...
* **Indexação por Idioma 🆕:** O campo `lang` foi adicionado ao chaveamento de cache, o que permite respostas distintas para o mesmo diff em idiomas diferentes.
...
* **Escopo por Repositório:** Todos os eventos de métricas são indexados por `repo_name`, o que permite o isolamento entre projetos.
...
* **Script de Sincronização 🆕:** `tests/sync_i18n.py` — script reutilizável para detetar chaves `__()` em qualquer ficheiro fonte que não possuam tradução no dicionário de idioma.
* **Novos Testes de Métricas 🆕:** `test_blame_metrics.py` (140 linhas) e `test_linter_metrics.py` (116 linhas) a cobrir a telemetria dos novos módulos.
...
| **Suíte de Testes** | 114 cenários (10 ficheiros) | **131 cenários (12 ficheiros + sync_i18n)** |
...
* **Sincronização i18n nos restantes idiomas:** Expansão das traduções para es_es, fr_fr e pt_pt com a mesma cobertura do pt_br (491 chaves).
