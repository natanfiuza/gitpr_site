# **🚀 Relatório de Estado do Projeto: GitPR CLI — v0.0.30 (2026-07-26)**
...
* **Testes:** Pytest + unittest.mock (8 ficheiros de teste, 165+ cenários).
...
* **Smart Excludes:** Filtro de pathspec inteligente (`gitpr.smart-excludes.json`) remoto — descarregado do GitHub e atualizado automaticamente com versionamento (`SMART_EXCLUDES_VERSION`), excluindo ficheiros irrelevantes (lock files, build artifacts, assets binários) para reduzir tokens.
...
* **Setup Inicial:** Detecta primeira execução, cria a pasta `~/.gitpr/`, e solicita interativamente as chaves de API, preferências e idioma, guardando num `.env`.
...
* **Comandos multilingues:** Ficheiros `chat_commands.{lang}.json` com traduções dos comandos slash.
...
* **Detecção Automática:** Detecta idioma do SO na primeira execução e guarda em `GITPR_LANG`.
...
* **Multilingue:** Thinking Words carregadas de templates específicos por idioma (`gitpr.thinking-words.{lang}.md`), com versionamento (`THINKING_WORDS_VERSION`).
...
* **Economia de Quota:** Retorna resultados em milissegundos do cache local (`~/.gitpr/cache/prompts/`).
* **Metadados de telemetria:** Campo `meta_raw` com contagem de tokens guardado junto ao cache.
...
* **Templates Remotos:** Descarregados do GitHub via `--skill` (nunca sobrescreve ficheiros locais existentes).
* **Multilingue:** Templates disponíveis em 5 idiomas com fallback inteligente (`get_skill_context()`).
* **Templates MCP Prompt 🆕:** 35 ficheiros (`gitpr.prompt.*.md`) em 5 idiomas no diretório `templates/`.
...
* **7 Prompts MCP com Templates:** Conteúdo externalizado em 35 ficheiros de template (7 prompts × 5 idiomas) no diretório `templates/gitpr.prompt.*.md` com fallback de idioma automático.
...
* **Recolha fire-and-forget:** Cada comando CLI gera um evento JSON assíncrono em `~/.gitpr/metrics/{owner}/{branch}/` com timestamp, comando, status, provider, tokens, duração, repo e branch.
...
* **Limpeza segura:** `gitpr --metrics --purge` remove ficheiros após confirmação do utilizador.
...
* **100% local e anónimo:** Nenhum dado sai da máquina. Eventos contêm metadados de uso, nunca conteúdo de ficheiros ou diffs.
...
* **Templates MCP:** 35 ficheiros de prompt (`gitpr.prompt.*.md`) em 5 idiomas no diretório `templates/`.
* **Thinking Words:** 263 entradas por idioma em `templates/gitpr.thinking-words.{lang}.md`.
* **Código limpo 🆕:** Todos os comentários e docstrings em português traduzidos para inglês nos ficheiros `src/metrics.py`, `src/main.py` e `src/ai_providers.py`.
...
| **MCP Prompts** | 7 prompts com templates (35 ficheiros) | 7 prompts com templates (35 ficheiros) |
| **Telemetria** | — (apenas map-reduce event) | **Sistema completo: recolha, export, purge, dashboard TUI** |
...
* **Plugin system:** Extensibilidade para regras de linter e prompts personalizados.
...
* **Dashboard com métricas de equipa:** Servidor HTTP opcional para dashboards browser-based a partir dos CSVs exportados.
