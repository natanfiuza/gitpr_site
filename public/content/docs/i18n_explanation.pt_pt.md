# Internacionalização (i18n) no GitPR — Guia do Programador

## Visão Geral

O GitPR utiliza um motor de internacionalização (i18n) personalizado inspirado no **helper `__()` do Laravel**. Todas as strings apresentadas ao utilizador são escritas em **inglês** como chaves, e as traduções são carregadas de ficheiros JSON em tempo de execução. O sistema deteta automaticamente o idioma do sistema operativo e usa o inglês como fallback quando não há tradução disponível.
...
| Ficheiro | Finalidade |
|---|---|
| `src/i18n.py` | Motor de tradução: função `__()`, deteção de idioma, carregamento de JSON |
...
| `~/.gitpr/langs/{lang_code}.json` | Cache local de traduções do utilizador (descarregado na primeira execução) |
...
2. get_system_language() deteta o locale do SO (ex: pt_BR, es_ES) ou lê GITPR_LANG do .env
3. get_translations() carrega o ficheiro JSON de ~/.gitpr/langs/{lang}.json
   - Se o ficheiro não existe ou está desatualizado (LANG_VERSION != __lang_version__) → descarrega do GitHub
   - Se o idioma for inglês → devolve dicionário vazio (não precisa de tradução)
   - Se o download falhar e existir ficheiro local → usa a versão em cache
4. O dicionário TRANSLATIONS é mantido em memória durante a sessão
5. Função __(): procura a chave → devolve a tradução (ou a própria chave como fallback)
```
...
- **Chave = fallback em inglês** — se não existir tradução, a string em inglês é apresentada diretamente
- **Placeholders nomeados** — suporta `str.format()` do Python com argumentos nomeados
- **Formatação segura** — se um placeholder estiver em falta, usa silenciosamente a string original

---

## Como usar `__()` no código

### Utilização básica (strings estáticas)
...
**⚠️ Importante:** Nunca uses `__()` para comparações de strings! A função devolve o valor traduzido (ex: português), o que quebraria as comparações. Em vez disso, usa uma lista de variações possíveis em ambos os idiomas:
...
1. Cria o ficheiro JSON: `langs/{lang_code}.json` (ex: `langs/es_ES.json`)
2. Adiciona todos os pares chave-valor com chaves em inglês e valores traduzidos
3. Faz commit do ficheiro — será servido de `https://raw.githubusercontent.com/natanfiuza/gitpr/main/langs/`
4. O motor i18n descarrega-o automaticamente na primeira utilização para esse locale

---

## Prioridade de deteção de idioma

1. **`.env` `GITPR_LANG`** — se definido, força um idioma específico (ex: `GITPR_LANG=pt_br`)
2. **Locale do SO** — detetado automaticamente via `locale.getdefaultlocale()` (ex: `pt_BR`, `es_ES`)
3. **Fallback** — `"en_us"` (inglês, não precisa de ficheiro de tradução)

Para forçar inglês: define `GITPR_LANG=en` no `~/.gitpr/.env` ou remove a variável.

---

## Controlo de versão das traduções

- `__lang_version__` no `src/updater.py` é incrementado quando as traduções mudam
- Em cada execução, se o `LANG_VERSION` local != `__lang_version__`, o ficheiro de tradução é novamente descarregado
- Isto garante que os utilizadores têm sempre as traduções mais recentes sem atualizações manuais
...
```python
# NÃO faças isto no updater.py ou cache.py:
from src.i18n import __
...
Ao adicionar novo texto apresentado ao utilizador:
