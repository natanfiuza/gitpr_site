...
`idempotentHint`) que ajudam IDEs e agentes de IA a entender o comportamento da
ferramenta rapidamente. Essas anotações permitem decisões de UI mais inteligentes —
...
via um objeto `ToolAnnotations`. Essas dicas não são impostas pelo servidor — são
metadados consultivos que o IDE/cliente pode usar para melhorar a experiência do
usuário.
...
|-------|------|-------------|
| `readOnlyHint` | `bool` | Se `true`, a ferramenta **não** modifica seu ambiente |
...
### Ferramentas Somente Leitura (sem efeitos colaterais)
...
escrevem ou excluem arquivos. São seguras para invocar sem aviso de operação
destrutiva:
...
> nenhuma delas modifica, exclui ou sobrescreve arquivos. Os "efeitos colaterais"
> se limitam a chamadas de API de rede.

## 🚀 Benefícios para Integração com IDEs

As anotações permitem que os editores:

- **VS Code / Cursor:** Mostrar ícone de escudo para ferramentas somente leitura,
  avisar antes de executar ferramentas marcadas como `destructiveHint=true`
- **Claude Desktop:** Organizar ferramentas em grupos seguro/inseguro na UI
...
As anotações são definidas via a classe `ToolAnnotations` em `src/mcp_server.py`:
...
A anotação de cada ferramenta é escolhida com base em seu comportamento real:
- **Somente leitura + idempotente** para ferramentas que apenas inspecionam estado local
...
**não são aplicadas** — elas são dicas para clientes MCP (IDEs e agentes de IA)
...
comportamento real (ex.: `run_linter` é `readOnlyHint` e de fato não modifica nada).

Veja [Integração MCP — Invocação Direta via CLI](/docs/mcp-integration?lang=pt_br#invocacao-direta-via-cli)
para detalhes completos de uso.

## 📚 Documentação Relacionada

- [Integração MCP](/docs/mcp-integration?lang=pt_br) — Como configurar o MCP para seu editor
- [MCP Prompts](/docs/mcp-prompts?lang=pt_br) — Templates de mensagem pré-definidos para fluxos comuns
