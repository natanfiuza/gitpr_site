...
| `SCRIPTS_LANG` | `~/.gitpr/.env` | Rastreia o idioma dos scripts instalados (ex.: `pt_pt`, `fr`) |
...
execução do gitpr
    ...
    │
    └─ Diferem ou ausentes? → Descarregar e instalar hooks no idioma atual
...
O inglês é o idioma padrão e de fallback. Se um script num idioma específico não for encontrado no servidor (HTTP 404), o sistema utiliza automaticamente a versão em inglês.
...
1. O GitPR deteta o idioma atual (`CURRENT_LANG`) do SO ou do `.env`
...
4. Se a versão diferir → os hooks são descarregados novamente no idioma atual
5. Se o idioma diferir → os hooks são descarregados novamente para corresponder ao novo idioma
...
| `SCRIPTS_LANG` | `~/.gitpr/.env` | Idioma dos scripts instalados (gerido automaticamente) |
...
- Elimine `SCRIPTS_LANG` do `.env` para forçar a redeteção do idioma
...
- **Marcador complementar `SCRIPTS_LANG`:** Impede a alternância de idioma quando utilizadores executam `gitpr --lang fr` uma vez — a sincronização automática não descarrega novamente a menos que a versão OU o idioma sejam diferentes
- **Abordagem de whitelist:** Apenas 4 sufixos explícitos (`pt_br`, `pt_pt`, `fr`, `es`) acionam descarregamentos específicos de idioma; qualquer outro idioma utiliza o inglês (sem cascata de 404)
