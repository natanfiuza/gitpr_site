...
* **📋 Issues Padronizadas (`-is` / `--issue`):** Gera um rascunho de Issue no formato **What / Why / Where / How** e abre uma TUI para edição ou publicação via API da forge configurada (GitHub, GitLab, Bitbucket; Azure DevOps não oferece endpoint universal de issues — gravação local). Possui **3 motores de contexto**: diff (padrão), histórico da branch (`-ht`) e blame (`-b file:lines`).  
...
* **🪄 Wizard de Forge SCM (`--init`):** Deteta a forge do repositório a partir do remote origin (GitHub, GitLab, Bitbucket Cloud, Azure DevOps), recolhe extras do provedor e o token de acesso, valida-o contra a API da forge e persiste a configuração cifrada (`GITPR_SCM_PROVIDER`, `GITPR_SCM_TOKEN_ENCRYPTED`, extras) — somente em caso de sucesso.
...
* **🔄 Auto-Update (`-u` / `--update`):** Consulta o PyPI por uma versão mais recente e bloqueia a execução até a ferramenta ser atualizada com `pip install --upgrade gitpr-cli`.  
...
Os cinco marcadores (`LANG_VERSION`, `SMART_EXCLUDES_VERSION`, `THINKING_WORDS_VERSION`, `LINTER_PRESETS_VERSION`, `SCRIPTS_VERSION`), o que cada um armazena em cache e a ordem correta para publicar uma alteração estão documentados em **[version-markers.md](/docs/version-markers?lang=pt_pt)**.

### **17. Sistema de Auto-Update**

O GitPR é distribuído exclusivamente pelo PyPI, e o módulo `updater.py` consulta a API JSON do PyPI (`pypi.org/pypi/gitpr-cli/json`) como fonte única da versão publicada. A verificação é colocada em cache diariamente (`~/.gitpr/update_cache.json`) e protegida por uma verificação de conexão (socket `8.8.8.8:53`) antes de qualquer operação de rede.

Quando a versão publicada é mais recente que `__version__`, o portão de arranque **bloqueia a execução**, apresenta o comando de atualização e termina com um estado diferente de zero — não existe fallback que mantenha uma versão desatualizada a funcionar. O portão é ignorado nos modos `--quiet`, `--hook` e `--mcp`, em `-u`/`--update` (o comando que explica como atualizar) e na ajuda contextual; quando a versão publicada não pode ser determinada (offline), a ferramenta é executada normalmente.
...
A publicação de PRs e issues já não é exclusiva do GitHub (em desenvolvimento após a v0.0.37). `src/infrastructure/scm/` abstrai as forges de alojamento Git sob um único contrato:

* **`base.py`** — ABC `ScmProvider` (11 métodos abstratos: criar/verificar/atualizar/fazer merge de PR, diff, listar PRs abertos, comentar, issues, teste de ligação, parsing de repositório), as dataclasses `RepoRef`/`PullRequestRequest`/`PullRequestResult`/`IssueRequest`/`IssueResult` e a hierarquia `ScmProviderError` / `ScmNotSupportedError`.
...
* **`factory.py`** — `resolve_scm_provider()` seleciona o provedor a partir da variável de ambiente `GITPR_SCM_PROVIDER` (padrão `github`, com fallback para o token legado `GITHUB_TOKEN_ENCRYPTED` — zero migração); `detect_provider_from_remote()` descobre a forge a partir do URL do remote origin.
* **Wizard `--init`** — `core.run_scm_init_wizard()` deteta a forge, recolhe extras do provedor (organização/projeto Azure, utilizador Bitbucket) e o token de acesso, valida-o via `test_connection()` (3 tentativas, novo pedido em 401) e persiste **apenas em caso de sucesso**: `GITPR_SCM_PROVIDER` + `GITPR_SCM_TOKEN_ENCRYPTED` (Fernet) + extras.
...
| Empacotamento | Pacote PyPI (wheel + sdist, `build`/`twine`) |
...
├── tui_issue.py      # Validação de token SCM (validate_or_request_scm_token) e entrada da TUI
├── metrics.py        # Telemetria offline (fire-and-forget, enriquecimento via cache)
├── infrastructure/   # Infraestrutura de domínio
│   └── scm/          # SCM multi-forge: base.py (ScmProvider ABC, dataclasses, erros),
│                     # provedores github/gitlab/bitbucket/azure_devops + factory.py
├── github_api.py     # Shim OBSOLETO → src/infrastructure/scm/github_provider.py
├── mcp_server.py     # Servidor MCP (stdio) + tools/resources/prompts + modo --tool
├── updater.py        # Verificação de versão (PyPI), bloqueio obrigatório e version markers
...
* [pull-request-publication.md](/docs/pull-request-publication?lang=pt_pt) — Publicador de PR (TUI, auto-commit, merge)  
* [pr-descricao-padrao.md](/docs/pr-descricao-padrao?lang=pt_pt) — Modo de descrição de PR padrão  
* [understanding_chat_functionality.md](/docs/understanding_chat_functionality?lang=pt_pt) — Chat de programação em par  
* [mcp-integration.md](/docs/mcp-integration?lang=pt_pt) — Integração MCP com editores  
* [mcp-annotations.md](/docs/mcp-annotations?lang=pt_pt) — Anotações das tools MCP  
* [mcp-prompts.md](/docs/mcp-prompts?lang=pt_pt) — Prompts predefinidos do MCP  
* [metricas-telemetria.md](/docs/metricas-telemetria?lang=pt_pt) — Métricas e telemetria local  
* [plugins-system.md](/docs/plugins-system?lang=pt_pt) — Sistema de plugins globais  
* [map-reduce-diff.md](/docs/map-reduce-diff?lang=pt_pt) — Map-reduce para diffs gigantes  
* [smart-excludes.md](/docs/smart-excludes?lang=pt_pt) — Otimização de tokens  
* [hooks-versioning.md](/docs/hooks-versioning?lang=pt_pt) — Versionamento e auto-sync dos hooks  
* [git-hooks-locais.md](/docs/git-hooks-locais?lang=pt_pt) — Guia de git hooks locais  
* [linter-regras-customizadas.md](/docs/linter-regras-customizadas?lang=pt_pt) — Regras de linter e linters externos  
* [guia-regex-gitpr.md](/docs/guia-regex-gitpr?lang=pt_pt) — Guia de regex para regras do linter  
* [github-ci-linter.md](/docs/github-ci-linter?lang=pt_pt) — Integração do linter com CI  
* [blame-arqueologo.md](/docs/blame-arqueologo?lang=pt_pt) — Arqueologia de código (git blame)  
* [issue-tui-help.md](/docs/issue-tui-help?lang=pt_pt) — Issues padronizadas e TUI  
* [gitpr-issue-option.md](/docs/gitpr-issue-option?lang=pt_pt) — Opções de geração de issues  
* [commit-message-ia.md](/docs/commit-message-ia?lang=pt_pt) — Mensagens de commit com IA  
* [code-review-ia.md](/docs/code-review-ia?lang=pt_pt) — Code review com IA  
* [install-wizard.md](/docs/install-wizard?lang=pt_pt) — Setup wizard  
* [i18n_explanation.md](/docs/i18n_explanation?lang=pt_pt) — Motor de i18n  
* [github-pat-integration.md](/docs/github-pat-integration?lang=pt_pt) — Segurança do GitHub PAT  
* [git-status.md](/docs/git-status?lang=pt_pt) — Listagem do estado dos ficheiros não commitados  
* [untracked-files.md](/docs/untracked-files?lang=pt_pt) — Explicação de ficheiros untracked  
* [auto-update.md](/docs/auto-update?lang=pt_pt) — Auto-atualizador e bloqueio obrigatório de atualização  
* [providers-ia.md](/docs/providers-ia?lang=pt_pt) — Provedores de IA (Gemini, DeepSeek, Ollama)  
* [skill-template.md](/docs/skill-template?lang=pt_pt) — Sistema de skills e templates  

Tutoriais (apenas em português):

* [github-issue-prompt-com-gh.md](/docs/github-issue-prompt-com-gh?lang=pt_pt) — Formatar e atualizar issues via gh CLI  
* [como_reverter_commit_git_localmente.md](/docs/como_reverter_commit_git_localmente?lang=pt_pt) — Reverter commits localmente  
* [testar_sem_usar_pypi.md](/docs/testar_sem_usar_pypi?lang=pt_pt) — Testar sem gastar uma versão no PyPI  
* [otimizacao-de-tokens.md](/docs/otimizacao-de-tokens?lang=pt_pt) — Otimização de tokens nos arquivos de contexto (.gitpr.*.md)  
