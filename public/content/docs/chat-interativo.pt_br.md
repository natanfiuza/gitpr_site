...
- **Histórico de mensagens** — Sua conversa é persistida por branch, permitindo continuar de onde parou.
...
- **Auto-Patching (F5)** — Extraia blocos de código sugeridos pela IA em um arquivo de patch.
- **Atualização de Diff (F2)** — Recarregue o `git diff` sem reiniciar a sessão.
- **Exportação de Sessão (F6)** — Salve o histórico completo do chat em Markdown.
...
👉 Para um guia completo, veja a [Documentação de Funcionalidade do Chat](/docs/understanding_chat_functionality?lang=pt_br).
