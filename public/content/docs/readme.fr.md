...
* `--linter-setup` : **Assistant interactif de linters externes.** Vous guide dans l'installation et la configuration de linters externes (ESLint, PHPCS, Stylelint) en tant que pont Checkstyle XML. 📖 [Documentation complète](https://github.com/natanfiuza/gitpr/blob/main/docs/linter-regras-customizadas.md)
...
### Linters Externes (Pont Checkstyle)

Si votre projet utilise déjà des outils comme ESLint, PHP_CodeSniffer ou Stylelint, GitPR peut servir de pont — en les exécutant en arrière-plan et en filtrant les erreurs **uniquement sur les lignes que vous avez modifiées** dans votre diff actuel. Tout linter émettant des rapports au format `checkstyle` est pris en charge.
...
L'assistant affiche des préréglages préconfigurés (PHPCS, ESLint, Stylelint — contrôlés à distance via `templates/gitpr.linter-presets.json`), vous guide à travers la commande d'installation native (ex. : `npm install --save-dev eslint`) et injecte le bloc `external_linters` correct dans votre `.gitpr.linter.yml`.

Chaque exécution — manuelle via `--linter` ou automatique avant les commits — consolide les Règles Regex et les Linters Externes dans un seul rapport Markdown enregistré dans `.gitpr/reports/linter/` (personnalisable via `OUTPUT_FILE_NAME_LINTER`).
