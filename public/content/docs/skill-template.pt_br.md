...
A tela `gitpr config` tem uma seção **Skills** que lista estes mesmos arquivos e abre cada um num editor, para ajustar uma regra sem sair do terminal ([Tela de Configuração Interativa](/docs/config-tui?lang=pt_br), §1.7).
...
Duas coisas ficam de fora dessa lista de propósito: o `.gitpr.linter.yml`, que nenhum comando carrega por esse mecanismo ([Regras Customizadas do Linter](/docs/linter-regras-customizadas?lang=pt_br)), e qualquer outro arquivo que você guarde em `.gitpr/skill/` — a tela oferece as skills que os comandos leem, não tudo o que a pasta contém.
