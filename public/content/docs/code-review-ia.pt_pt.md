...
Corre o mesmo motor, a mesma skill e a mesma cache que o `gitpr -r`, por isso o seu relatório lê como um review local do mesmo diff. Veja a [documentação de Review de Pull Request Remoto](/docs/review-pr?lang=pt_pt) para o contrato completo — as recusas que não custam tokens, o filtro de smart excludes, o escopo de cache e o rodapé do comentário.
...
> **Nota:** Consulte também a [documentação do Linter](/docs/linter-regras-customizadas?lang=pt_pt) para criar regras de validação estática.
